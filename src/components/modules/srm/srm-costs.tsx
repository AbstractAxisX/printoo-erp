...
export function SRMCosts() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["material-costs", "srm-list"],
  ...
  const { data: supData } = useQuery({
    queryKey: ["suppliers", "srm", "for-costs"],
  ...
  const filteredCosts = React.useMemo(() => {
    return allCosts.filter((c) => {
      if (search.trim()) {
        ...
        const supName = (c.supplier?.name ?? "").toLowerCase();
        if (!num.includes(q) && !desc.includes(q) && !supName.includes(q)) return false;
      }
      ...
      // Supplier filter — match by name since MaterialCost only carries the supplier's name
      if (supplierFilter !== "all") {
        if (supplierFilter === "none") {
          // costs with no supplier attached
          if (c.supplier !== null) return false;
        } else {
          const sel = suppliers.find((s) => s.id === supplierFilter);
          if (!sel || c.supplier?.name !== sel.name) return false;
        }
      }
      return true;
    });
  }, [allCosts, search, statusFilters, moduleFilters, supplierFilter, suppliers]);
  ...
  // Columns
  const columns: ColumnDef<MaterialCost>[] = [
    {
      id: "number",
      accessorKey: "number",
      header: "شماره سفارش",
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-1.5">
            <div className="size-6 rounded-full bg-orange-500/10 text-orange-600 dark:text-orange-400 grid place-items-center text-[10px] font-bold shrink-0">
