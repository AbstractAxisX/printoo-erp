    ...
    <Card className="p-0 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3.5 border-b">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-sm">{t("سفارشات نزدیک به سررسید (5 روز)")}</h3>
        </div>
      ...
      {isLoading ? (
        <div className="py-10 text-center text-sm text-muted-foreground">{t("در حال بارگذاری...")}</div>
      ) : orders.length === 0 ? (
        <EmptyState icon="checkCircle" title={t("سفارش نزدیک به سررسیدی نیست")} description={t("همه چیز تحت کنترل است")} />
      ) : (
            ...
            return (
              <button key={o.id} onClick={() => openOrder(o.id)} className="w-full flex items-center gap-3 px-5 py-2.5 hover:bg-accent/40 transition text-right">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">{t("{p0} آیتم • {p1}", { p0: o.items?.length ?? 0, p1: formatDate(o.endDate) })}</div>
                </div>
                <div className={cn(
...
export function LatestTasks() {
  ...
  const statusLabel: Record<string, string> = { todo: t("در صف"), in_progress: t("در حال انجام"), done: t("انجام شده") };
    ...
    <Card className="p-0 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3.5 border-b">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-sm">{t("آخرین تسک‌ها")}</h3>
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState icon="task" title={t("تسکی وجود ندارد")} />
      ) : (
        <div className="divide-y">
          {tasks.map((t) => (
    ...
    <Card className="p-0 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3.5 border-b">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-sm">{t("آخرین سفارشات")}</h3>
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState icon="orders" title={t("سفارشی ثبت نشده")} />
      ) : (
        <div className="divide-y">
          {orders.map((o) => (
            <button key={o.id} onClick={() => openOrder(o.id)} className="w-full flex items-center gap-3 px-5 py-2.5 hover:bg-accent/40 transition text-right">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground">{t("{p0} آیتم • {p1}", { p0: o.items?.length ?? 0, p1: relativeTime(o.createdAt) })}</div>
              </div>
