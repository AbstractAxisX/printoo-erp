...
export function TabBar() {
  ...
  if (headerCollapsed) return null; // hidden when header is collapsed
  ...
  return (
    <div className="flex items-stretch h-9 border-b bg-muted/20 overflow-x-auto scrollbar-thin">
      <div className="flex items-stretch gap-0.5 px-1.5 flex-1 min-w-0">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <Icon name={tab.icon as "dashboard"} size={13} className={active ? "text-primary" : "text-muted-foreground"} />
              <span className="truncate max-w-[100px]">{tab.label}</span>
              <span className="text-[9px] text-muted-foreground/60 hidden sm:inline">{mod.faLabel.split(" ")[0]}</span>
              <span
                role="button"
                tabIndex={0}
