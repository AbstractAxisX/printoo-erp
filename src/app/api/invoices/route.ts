...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let computed;
    try {
      // Phase 22 (خواستهٔ 6+10): اگر سفارش هدیهٔ فعال دارد، هدیه داخل
    ...
    const invoice = await db.$transaction(async (tx) => {
      ...
      // Phase 22 (خواستهٔ 10): تخفیف/مالیات فاکتور روی دادهٔ مالی سفارش هم
      // بنشیند — total فاکتور = total سفارش (بدهی مشتری، بستانکار، 360،
