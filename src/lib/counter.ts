...
//
// دو لایهٔ دفاعی:
//  1) ensureCounters() — قبل از تراکنش صدا زده می‌شود؛ شمارندهٔ غایب را
//     با max موجود و شمارندهٔ «عقب‌مانده» را به max موجود ترمیم می‌کند.
//  2) nextNumber() — حتی اگر شمارنده خراب باشد، شمارهٔ اشغال‌شده را
...
export async function nextNumber(
  ...
  // لایهٔ دفاعی 2: شمارندهٔ کهنه/خراب ممکن است شمارهٔ تکراری بدهد؛
  // آن‌قدر جلو برو تا شمارهٔ واقعاً آزاد پیدا شود (سقف 10٬000 برای اطمینان).
  let guard = 0;
...
/**
 * لایهٔ دفاعی 1 — ترمیم شمارنده‌ها بر اساس max موجود (یک‌بار در هر پروسه).
