...
---
Task ID: 16-7-a
Agent: general-purpose (payroll pages)
Task: صفحات حقوق و دستمزد مالی + تحلیل + سادهٔ مدیر سیستم

Work Log:
- Worklog فازهای ۱۵/۱۶ + قراردادهای API خوانده شد؛ روت‌های payroll (GET /api/payroll?periodId=، PUT/POST entries/[id](/pay)، POST periods/[id]/pay، POST/DELETE advances(/[id])، GET analytics?from=&to=) با curl روی سرور زنده تأیید شد (لاگ dev: همهٔ ۲۰۰/۲۰۱). رفرنس‌ها: finance-dashboard (KPI/TimeRangePicker)، finance-costs (DataTable/ColumnDef)، shared/index (PageHeader/EmptyState/LoadingState)، lib/format + constants (MODULES/USER_ROLE) + icons + data-table + time-range-picker؛ الگوی چیپ ماژول از monitoring-users (MODULE_COLORS محلی — constants فقط color-name دارد).
- src/components/modules/finance/payroll-page.tsx (۱۵۰۸ خط، export PayrollPage) بازنویسی کامل: queryKey ["payroll", periodId] ("" = جاری؛ سرور ensure می‌کند)؛ نوار چیپ دوره‌ها مرتب نزولی (active+open=emerald پررنگ با بج «جاری»، پرداخت‌شده=muted؛ سوییچ دوره پالت ویرایش/دیالوگ‌ها را ریست می‌کند)؛ بنر فقط-خواندنی برای دورهٔ paid/closed با پرداخت‌کننده+تاریخ.
- قلب صفحه — جدول ورودی‌ها (۱۱ ستون، min-w-[1180px] اسکرول افقی): ویرایش اینلاین فقط وقتی دورهٔ open و ردیف draft؛ NumInput محلی با نمایش گروه‌بندی‌شدهٔ هزارگان (en-US، inputMode=numeric، focus-s sync)؛ حقوق پایه + چک‌باکس «قرارداد» (updateContract)؛ اضافه‌کاری دو اینپوت ساعت×نرخ با جمع زنده؛ پاداش/کمکرد/بیمه/مالیات با map فیلدها؛ کسر مساعده با clamp سمت کلاینت به pendingAdvanceSum + چیپ «مساعدهٔ مانده»؛ خالصِ زنده با فرمول سرور (رنگ emerald/قرمزِ ≤0 و بک‌گراند سلول)؛ وضعیت draft=آماده / paid=پرداخت‌شده با تولتیپ paidAt؛ عملیات: یادداشت جمع‌شونده (ردیف colSpan=11)، ذخیره per-row (PUT، فقط dirty فعال — دلتا-مِج mergedVals/rowDirty)، پرداخت (confirm AlertDialog با خالصِ فرمول‌شده؛ دکمه تا ذخیره‌نشده قفل)، ردیف paid → چیپ «سند هزینه» با کلیک → navigate("finance","costs")؛ tfoot جمع زندهٔ همه ستون‌ها؛ مرتب‌سازی draft-اول-سپس-نام.
- نوار «پرداخت کل دوره» (emerald، مخفی وقتی دوره paid) با AlertDialog (Σ خالص draft + هشدار قفل/رد خالص≤0) → POST periods/{id}/pay → toast از message سرور (شامل skipped).
- پنل مساعده‌ها (AdvancesPanel با فرم اینلاین Select+NumInput+note → POST advances؛ جدول: کارمند+چیپ ماژول، مبلغ، ثبت‌کننده+formatDateTime، وضعیت کسرنشده amber «کسر در دورهٔ بعد» / کسرشده emerald + دورهٔ کسر در تولتیپ، حذف فقط کسرنشده با AlertDialog → DELETE؛ 409→toast e.message). PeriodsHistory: دوره/بازه/تعداد/جمع خالص/پرداخت‌کننده+تاریخ/وضعیت چیپ.
- src/components/modules/finance/payroll-analytics-page.tsx (۶۱۳ خط، export PayrollAnalyticsPage): ["payroll-analytics", from, to] → GET analytics؛ TimeRangePicker الگوی داشبورد مالی — پیش‌فرض getPreset("this-year") (پریست «last-6-months» در lib/time-ranges موجود نیست؛ سالانه برای دید ماهانهٔ بهتر) + refresh.
- ۵ کارت KPI (جمع پرداختی emerald/میانگین ماهانه teal/ماه‌های پرداخت violet/کارمندان rose/مساعدهٔ کسرنشده amber با hint تعداد) + کارت MoM (lastDelta): ±٪ فارسی با آیکون arrowUp/Down، دو میلهٔ نرمال‌شده (current vs prev)؛ حالت بدون داده (< ۲ ماه). نمودار روند: recharts BarChart monthly.paidSum (fill #10b981، CartesianGrid، XAxis با faDigits(key)، YAxis compactTick M/K، تولتیپ سفارشی contentStyle direction:rtl + labelFormatter «دوره + N ورودی»، ResponsiveContainer h-60، isAnimationActive=false — عین user-monitoring).
- تفکیک ماژول: کارت‌های گرید با ModuleChip (faLabel از MODULES) + جمع + تعداد + نوار سهم٪ (رنگ میلهٔ هر ماژول) + چیپ نکته «کارمندان چند-ماژولی در هر ماژول شمرده می‌شوند»؛ ماژول «none» → «بدون ماژول». جدول کارمندان: DataTable ColumnDef (کارمند+چیپ‌ها/جمع بازه/ماه‌ها/میانگین/آخرین پرداخت formatDate) با جستجوی کلیِ q بالای جدول، dense، ردیف‌کلیک فقط برای master → navigate("sysadmin","user",userId). دو کارت لینک پایین: تاریخچه هزینه‌ها + مدیریت دورهٔ جاری (keyboard هم قابل کلیک).
- src/components/modules/sysadmin/payroll-simple-page.tsx (۵۸۶ خط، export PayrollSimplePage): «حقوق بده و برو» — queryKey ["payroll","current"] (همان prefix پس از invalidate(["payroll"]) رفرش می‌شود)؛ هدر با دورهٔ جاری + چیپ وضعیت؛ بنر «ویرایش ارقام در ماژول مالی انجام می‌شود»؛ نوار ۴ کارتی (Σ خالص/پرداخت‌شده/مانده/تعداد)؛ جدول فقط-خواندنی (کارمند+چیپ/خالص liveNet برای draft — netPay ذخیره‌شده فقط بعد از ذخیره معتبر است/وضعیت/دکمهٔ پرداخت فقط draft+open)؛ دکمهٔ بزرگ «پرداخت حقوق کل دوره» با AlertDialog (مخفی وقتی paid)؛ لیست دوره‌های گذشته فقط-خواندنی؛ بدون هیچ ویرایش/مساعده/تحلیل.
- همهٔ mutationها: toast.success(message سرور) / toast.error(e.message) + invalidate(["payroll"]) و invalidate(["finance"]) (پرداخت‌ها سند هزینه می‌سازند). اعداد فارسی fa-IR برای شمارش‌ها، formatCurrency/formatNumber برای مبالغ، تاریخ‌ها میلادی lib/format، بدون any، "use client".
- تایید: `bunx tsc --noEmit | grep -E "payroll-page|payroll-analytics|payroll-simple"` → خالی (۰ خطا)؛ eslint هر سه فایل → ۰ error/۰ warning؛ کامپایل باندل کلاینت (curl GET / → 200، compile 1.3s بدون خطای payroll). خطوط: ۱۵۰۸ + ۶۱۳ + ۵۸۶ = ۲۷۰۷.

Stage Summary:
- سه صفحهٔ فاز ۱۶ کامل و آمادهٔ تست مرورگر (سیم‌کشی module-router/nav از قبل بود): مالی فول (دوره‌ها/ویرایش اینلاین/پرداخت رگی+کل دوره/مساعده/تاریخچه)، تحلیل (KPI+MoM+BarChart+ماژول+کارمندان)، سادهٔ مستر (پرداخت سریع).
- تصمیم‌ها: نمایش خالص ردیف‌های draft همیشه از فرمول زنده (netPay دیتابیس تا اولین ذخیره صفر است) — در دیالوگ‌های پرداخت هم خالص فرمول‌شده؛ دکمهٔ پرداخت تا dirty قفل (ابتدا ذخیره)؛ سقف کسر مساعده کلاینت-side clamp + سرور 400؛ note با رشتهٔ خالی پاک می‌شود؛ پیش‌فرض بازهٔ تحلیل this-year چون پریست last-6-months در سیستم نیست.
- نکتهٔ طول فایل: payroll-page عمداً ۱۵۰۸ خط شد (جدول ۱۱ستونهٔ ویرایش‌پذیر + دو دیالوگ + پنل مساعده + تاریخچه) — بالاتر از هدف ~۹۰۰ اما فشرده و سکشن‌بندی‌شده؛ دو فایل دیگر در محدوده.
- باقی‌مانده برای ارکستراتور: تست مرورگر (negar@printoo24.com برای مالی، master برای ساده)، کامیت/پush.

---
Task ID: 16-7-b
Agent: general-purpose (warehouse pages)
Task: صفحات انبار — داشبورد، بسته‌بندی/بج، موجودی، نمای عمومی بسته

Work Log:
- خواندن worklog (فازهای ۱۴/۱۵ برای الگوهای KPI کلیک‌شون/boardFilter/فرم اینلاین) + مراجع: print-dashboard، finance-dashboard، print-order-detail (مودال عریض تب‌دار)، logistics-orders، cost-entry-form، shared/index، data-table، icons، format/constants، lib/package-badge.ts (makePackageQr/downloadBadgePdf/downloadBadgePng/packagePublicUrl) و همهٔ routeهای فاز ۱۶ (packages, packages/[id], warehouse/stats, materials CRUD+moves, public/packages/[code]) — تایپ‌ها عین پاسخ زندهٔ سرور نوشته شد.
- warehouse-dashboard.tsx (۲۹۹ خط): GET /api/warehouse/stats (۳۰s) → ۶ کارت KPI کلیک‌شون با setBoardFilter+navigate (pendingPackItems/amber، packagesPacking/amber، packagesReady/teal، packagesSent/violet، deliveredToday/emerald، codMonth/rose با formatCurrency→orders)، کارت مواد کم‌موجود (نوار پیشرفت quantity/min + قرمز، کلیک→inventory)، اقدام‌های سریع «بسته جدید»/«سفارشات تحویل» + دکمهٔ refresh.
- packages-page.tsx (۱۸۴۲ خط): فرم اینلاین «بسته جدید» (Colllapsible، دسکتاپ باز) — سفارش‌های packable با چک‌باکس آیتم‌ها (بسته‌شده‌ها disable با چیپ «در بستهٔ PKG-X»)، آدرس خودکار از اولین سفارش انتخابی + شرح پیشنهادی «۲× کارت ویزیت + ۱× تراکت» (هر دو قابل ویرایش، منطق touched)، فرم مقصد ۲ستونه (گیرنده/پیک/پیگیری/COD/یادداشت)، نوار جمع «N سفارش · M قلم · COD» + POST → invalidate + باز‌شدن خودکار مودال روی تب بج؛ فیلتر چیپ‌های وضعیت با شمارندهٔ زنده (کوئری ["packages","all"]) + جستجوی سرور q با debounce ۴۰۰ms؛ جدول DataTable ۹ ستونه (کد mono ltr + #seq، چیپ سفارش‌ها، مشتری، آدرس truncate، اقلام با Tooltip نام محصولات، پیک/پیگیری، COD amber + ✓، چیپ وضعیت رنگی، تاریخ، مشاهده/بج)؛ مودال عریض max-w-5xl با ۳ تب: جزئیات (تایم‌لاین ۴مرحله‌ای با تاریخ، نوار اقدام وضعیت: packing→ready / ready→send-دیالوگ پیک+پیگیری + لغو AlertDialog / sent→deliver-دیالوگ گیرنده+سوییچ «پول در محل دریافت شد؟» پیش‌فرض true وقتی COD>0 + مرجوعی / delivered→خلاصهٔ فقط-خواندنی، شبکهٔ اطلاعات + سفارش‌ها)، محتویات (کارت per-order: در این بسته + بازشوندهٔ «همهٔ اقلام سفارش» با چیپ مرحله)، بج و QR (پیش‌نمایش makePackageQr در useEffect، لینک عمومی ltr + کپی clipboard، دانلود PDF/PNG با حالت loading، یادداشت «سایز بج: ۱۰۰×۵۰ میلی‌متر»). همهٔ PATCHها invalidate ["packages","warehouse","orders","revenues","finance"].
- inventory-page.tsx (۸۴۲ خط): GET /api/materials → نوار KPI (تعداد/کم‌موجودها قرمز/جمع اقلام)؛ فرم اینلاین افزودن ماده (نام/واحد Select ۷گزینه/حداقل/یادداشت → POST)؛ جدول مواد (موجودی bold tabular + نوار قرمز quantity در برابر min×2 وقتی کم، حداقل، چیپ کم‌موجود/کافی، آخرین گردش reason+relativeTime، ورود/خروج → دیالوگ مقدار+دلیل با پیش‌نمایش «موجودی بعد از ثبت» و گارد منفی + ویرایش PUT)؛ بخش بازشوندهٔ «گردش انبار» (GET /api/materials/moves?limit=100، جدول ماده/نوع ورود-خروج/مقدار/دلیل/ثبت‌کننده/تاریخ + فیلتر Select ماده).
- public-package-view.tsx (۴۶۵ خط): صفحهٔ عمومی standalone بدون پوسته (fetch ساده، نه api()) — کارت max-w-3xl روی گرادیان، سربرگ برند (SVG inline همان لوگوی بج + پرینتو۲۴ + تلفن click-to-call)، هیرو کد mono + وضعیت بزرگ فارسی، استپر ۴مرحله‌ای با تاریخ (لغو→بنر رز)، شبکهٔ اطلاعات (آدرس/گیرنده/محتویات/پیک+پیگیری/COD amber «این مبلغ هنگام تحویل دریافت می‌شود» یا emerald «دریافت شد»)، کارت هر سفارش (مشتری/تلفن/آدرس/اقلام/StatusBadge)، فوتر «Printoo24 — سامانه مدیریت چاپ» + دکمهٔ «ورود به سامانه» → "/"; skeleton لودینگ + حالت خطای «بسته یافت نشد» + ریسپانسیو موبایل.
- راستی‌آزمایی: tsc --noEmit → صفر خطا در هر ۴ فایل (خطاهای پیش‌موجود فقط examples/ و skills/)؛ eslint هر ۴ فایل → ۰ error ۰ warning؛ curl همهٔ endpointها (stats/packages/detail/packable/materials/moves/public) با لاگین hossein → ۲۰۰ و منطبق با تایپ‌ها؛ E2E مرورگری (agent-browser): ساخت بسته PKG-N24GKA از دو قلم + COD 25000 → مودال بج با QR data-url → آمادهٔ ارسال → ارسال (پیک تستی اربیل/TRK-90001) → تحویل (خانم رزگار + سوییچ COD) → توست «۲ سفارش تکمیل شد — پول در محل ۲۵٬۰۰۰ دینار»؛ صفحهٔ عمومی /?pkg=PKG-N24GKA کامل؛ موجودی: ورود ۳ کیلوگرم چسب دوقلو → «موجودی جدید: ۷» + جدول live + گردش‌ها با فیلتر؛ جستجوی سرور «پیک تستی» + چیپ وضعیت هم‌زمان کار کرد؛ VLM روی ۴ اسکرین‌شات (داشبورد/عمومی/محتویات مودال/موبایل ۳۹۰px/موجودی) → NO ISSUES؛ کنسول/خطای صفحه صفر.

Stage Summary:
- ماژول انبار کامل شد: داشبورد فیلتردار + بسته‌بندی اینلاین (بدون دیالوگ) + بج QR قابل دانلود PDF/PNG + جریان ارسال/تحویل با COD (توزیع FIFO روی سفارش‌ها توسط سرور) + موجودی/گردش مواد + صفحهٔ عمومی QR برای مشتری/پیک.
...
Task ID: P17-DEPLOY
Agent: orchestrator (main)
Task: تست یکپارچه + کامیت/Push + استقرار فاز ۱۷ روی سرور 187.124.27.96

Work Log:
- یکپارچه‌سازی ۴ زیرایجنت: tsc کامل → صفر خطا (بیرون examples/skills)؛ eslint → ۰ error ۷ warning پیش‌موجود؛ حذف eslint-disable بلااستفاده در safe-uuid.
- ری‌استارت تمیز dev + تست مرورگر خودم (ادمین): کارت «مشتریان تسویه‌نکرده» ۱۷/Σ۱۹۶.۵M → کلیک → صفحه مشتریان با فیلتر تسویه‌نشده + چیپ مانده کنار اسم + پروندهٔ مشتری (۴ تایل + ۳ تب)؛ مالی: مودال ۳ تب + فرم هزینه + کارت قفل خروج؛ چاپ (رضا): بدون داشبورد + ۶ کارت KPI + فیلتر + «با این فیلتر: ۱ چاپ و ۰ متریال»؛ پنل نوتیف: چیپ‌های فیلتر + همه-را-خواندم + تاریخ جلالی. dev.log پاک.
- کامیت e29c5b4 (۳۳ فایل، +۲۶۷۵/−۸۹۸) + push به GitHub.
- استقرار: tarball کامل سورس بدون db → سرور؛ prisma generate + db push (city/province/invoiceWithPackage/NotificationRead روی دیتای زندهٔ کارفرما — additive و امن)؛ بیلد production (✓ 47s)؛ ری‌استارت سرویس.
- تایید سرور: login 200؛ kpis.unsettledCustomers={3, Σ7,183,200} (کارفرما امروز ۳ مشتری واقعی ساخته!) و profit حذف شده؛ customers با فیلدهای جدید؛ notifications ۱۶ با unread per-user + read-all 200؛ مرورگر: کارت → صفحه مشتریان (kaka hama ۶.۶M بدهی)، چاپ بدون داشبورد + کارت‌ها (۲ در حال چاپ)، پنل نوتیف با رویدادهای واقعی امروز، مودال مالی ۳ تب + کارت قفل + دکمهٔ «ارسال فاکتور همراه بسته». صفر خطای کنسول.
...
Task ID: 20-D
Agent: subagent-D (general-purpose)
Task: کانبان سفارشات باز (خواستهٔ ۴) + تب‌های جنبه‌ای بالای هر دو جدول سفارشات (خواستهٔ ۵) — فاز ۲۰

Work Log:
- worklog خوانده شد؛ الگوها مطالعه شد: tasks-page.tsx (الگوی اثبات‌شدهٔ dnd-kit: DndContext + PointerSensor(distance 8→6) + useDroppable/useSortable + DragOverlay + statusOverride خوش‌بینانه + rollback)، orders-columns/orders-page/use-orders-query/types، open-orders(.tsx/-helpers)، GET /api/orders (requireUser + board scoping + فیلدهای تخت Phase 17-A)، PUT /api/orders/[id]/status (requireManager + syncItemsToStatus)، ORDER_STATUS در lib/constants (label/colors)، icons.tsx (فقط نام‌های موجود: layers/orders/coins/file/info برای تب‌ها + grid/dashboard برای سوییچ نما — paperclip/kanban وجود ندارند).
- (۱) api/orders/route.ts GET: پارامتر ?withAggregates=1 (opt-in) — بعد از fetch سفارش‌ها، یک کوئری جمعی db.materialCost.findMany({orderId in ids, select:{orderId,amount,status,_count:{attachments}}}) + reduce در JS به Map و merge در پاسخ. قرارداد: costsCount/costsTotal = فقط هزینه‌های غیرردشده؛ costsApproved (status=approved) و costsPending (pending) Σ جدا؛ attachmentsCount = Σ پیوست‌های همهٔ هزینه‌های سفارش (شامل ردشده). سفارش بدون هزینه → صفرهای صریح. بدون پارامتر → پاسخ عین قبل (برد‌های طراح/چاپ/تقویم/مالی دست‌نخورده) — curl-تست شد.
- (۲) NEW orders/order-table-tabs.tsx: OrderTableTab = "all"|"items"|"costs"|"attachments"|"notes" + نوار تب سگمنتی جمع‌وجور (h-9، active = bg-primary text-primary-foreground، role=tablist با aria-selected، آیکون: layers/orders/coins/file/info).
- (۳) NEW orders/columns-by-tab.tsx: کارخانهٔ ستون Generic روی OrderTabRow (حداقل‌شکل مشترک Order و OpenOrder — items/note/aggregates اختیاری)؛ ستون‌های مشترک ابتدای همهٔ تب‌ها: شماره (#N mono) + مشتری (نام+تلفن). تب آیتم‌ها: تعداد آیتم | آیتم‌ها (چیپ max 2 + «+N») | در طراحی (چیپ بنفش count) | مبلغ کل. تب هزینه‌ها: تعداد هزینه | جمع هزینه | تاییدشده (emerald) | در انتظار (amber) | سود برآوردی = totalAmount−costsApproved (emerald مثبت/rose منفی/muted صفر، tabular-nums ltr). تب پیوست‌ها: تعداد پیوست (آیکون file + شمارش فارسی، «—» اگر صفر) | جمع هزینه | مبلغ کل. تب یادداشت‌ها: یادداشت سفارش (truncate max-w-220، «—») | یادداشت آیتم‌ها (چیپ «N یادداشت» + گزیدهٔ اولین). همهٔ ستون‌ها sortable (به‌جز چیپ‌ها) با همان استایل جدول (formatCurrency).
- (۴) orders/types.ts: Order += costsCount/costsTotal/costsApproved/costsPending/attachmentsCount (اختیاری)؛ use-orders-query.ts: آرگومان withAggregates (→ queryKey + params) — فقط orders-page مصرفش می‌کند (بررسی شد)؛ orders-page.tsx: withAggregates:true + state تب + نوار OrderTableTabs داخل کارت بالای DataTable + سواپ ستون‌ها (activeColumns)؛ «همه» = عین ستون‌های کامل امروز شامل ردیف بازشوندهٔ گروهی (getRowCanExpand/renderExpandedRow/defaultHidden فقط در «همه») + اکشن‌ها؛ بقیه تب‌ها ردیف ساده با همان onRowClick → مودال جزئیات.
- (۵) open-orders-helpers.ts: OpenOrder += note (سطح سفارش) + note آیتم + ۵ فیلد تجمیعی (additive، API همین‌ها را برمی‌گرداند). open-orders.tsx: state view ("table"|"kanban") + tableTab؛ fetch += withAggregates=1؛ سوییچ نمای جدول/کانبان بالای stage tab-cards (grid/dashboard، سگمنتی h-9) — سوییچ به کانبان selectedStage را به «همه» برمی‌گرداند (ستون‌ها خودِ مراحل‌اند؛ کارت‌های آماری/فیلترها با برد سازگار می‌مانند)؛ stage tab-cards در کانبان hidden؛ در نمای جدول همان نوار OrderTableTabs بالای DataTable + activeColumns (useMemo روی tableTab+columns). کانبان فایل جدید:
- (۶) NEW open-orders-kanban.tsx: برد dnd-kit عین tasks-page — ۳ ستون pending_design/in_printing/warehouse_logistics (label/dot از ORDER_STATUS؛ violet/amber/cyan)؛ برد flex gap-4 overflow-x-auto pb-4، هر ستون min-w-[300px] flex-1 با هدر (نقطهٔ رنگی + برچسب + Σ مبلغ tabular-nums + شمارش) و بدنهٔ max-h-[70vh] overflow-y-auto scrollbar-thin؛ کارت: #N mono بولد + بج «فوری» (border-s-rose-500) + مبلغ (ltr)، مشتری، چیپ آیتم‌ها (max 2 + +N)، چیپ موعد مرحله (getStageDeadline با stage=all → موعد مرحلهٔ فعلی خود سفارش؛ رنگ rose/amber/muted بر اساس daysRemaining). درگ بین ستون‌ها → PUT /api/orders/[id]/status {status} با آپدیت خوش‌بینانهٔ statusOverride + حذف override بعد از sync سرور؛ موفق → invalidate(["open-orders","orders"]) + toast «سفارش #N به «label» منتقل شد»؛ خطا (هر کد، از جمله 409) → rollback + toast پیام فارسی سرور. کلیک کارت بدون درگ → مودال جزئیات (setSelectedOrderId). DragOverlay با پیش‌نمایش چرخیده. PointerSensor activationConstraint distance 6.
- راستی‌آزمایی: tsc --noEmit → صفر خطا (کل src)؛ bun run lint → ۰ error / ۶ warning همگی پیش‌موجود (data-table/virtualized-data-table/gantt/unused-disables فایل‌های دیگر)؛ eslint تک‌تک ۹ فایل من → ۰/۰.
- curl (سشن ادمین): GET ?withAggregates=1 → ۲۹ سفارش با فیلدهای تجمیعی صحیح (#7: costsCount=1/total=1000/pending=1000؛ #1: 2/1170000/approved=1170000؛ #6: 1200000 approved؛ بدون پارامتر → فیلدها غایب + customerName/preInvoiceCount حفظ شد)؛ کراس‌چک مستقیم با Prisma (۱۴ هزینه، ۱ ردشده که از counts حذف شد؛ relation attachments + CostAttachment create/delete تست شد). PUT status برای revert → {ok:true}.
- E2E مرورگر (agent-browser سشن p20d، ادمین): سفارشات باز → سوییچ «نمای کانبان» → ۳ ستون (۹/۵/۸ = ۲۲ سفارش باز، Σ مبلغ هر ستون) و stage tab-cards مخفی؛ کارت‌ها با بج فوری/+N/موعد رنگی؛ درگ کارت #7 از «در حال طراحی» به «در حال چاپ» → جابجایی فوری (خوش‌بینانه) + toast + API status=in_printing + شمارش ستون‌ها ۸/۶؛ بعد از revert (curl) و رفرش ۳۰ثانیه‌ای → override افتاد و ۹/۵ برگشت؛ کلیک کارت → مودال «سفارش #7 — داروخانه سبز»؛ موبایل 390px → scrollWidth=390 (بدون overflow، ستون‌ها min-w-300 اسکرول افقی). نمای جدول: تب‌ها همه/آیتم‌ها/هزینه‌ها/پیوست‌ها/یادداشت‌ها همه تست شد (#7: ۱ هزینه/1,000/0/1,000/سود 960,000 — منطبق بر API). همه سفارشات: «همه» = ستون‌های کامل + شورون گروهی #6 (۵ آیتم expand شد)؛ تب هزینه‌ها ستون‌ها/داده درست؛ کلیک ردیف در تب هزینه‌ها → مودال جزئیات. صفر خطای کنسول در جریان کامل (بارگیری/کانبان/refetch/کلیک/مودال). اسکرین‌شات‌ها: .zscripts/p20d-{kanban,kanban-final,kanban-mobile,open-costs-tab,all-page}.png.
- داده: #7 بعد از تست درگ با PUT به pending_design برگردانده شد و تأیید نهایی شد (هم API هم رندر برد). دو نوتیف + دو رویداد status_changed (درگ + revert) از خودِ endpoint ساخته شد — رفتار ذاتی همان مودال تغییر وضعیت است.
- نکتهٔ محیط: dev server دو بار در حین تست OOM-kill شد (فشار مرورگرهای موازی — مشکل شناخته‌شدهٔ سندباکس)؛ هر بار با bun run dev بک‌گراند بالا آمد و روشن ماند؛ خطای گذرای compile وسط تست («@@/lib/constants» خودم فوری فیکس شد؛ parsing error روزِ day-detail-modal مربوط به ایجنت موازی بود). git دست نخورد.

Stage Summary:
- سفارشات باز دو نما دارد: «نمای جدول» (همیشهٔ قبل + تب‌های جنبه‌ای) و «نمای کانبان» — بورد ۳ ستونهٔ drag&drop با آپدیت خوش‌بینانه، toast فارسی، rollback روی خطا و باز شدن مودال با کلیک ساده؛ کارت‌های آماری/جستجو در هر دو نما می‌مانند و stage tab-cards فقط در جدول.
- هر دو جدول سفارشات (همه سفارشات + سفارشات باز) نوار تب هم‌نام مودال جزئیات دارند: «همه» = ستون‌های کامل امروز؛ آیتم‌ها/هزینه‌ها/پیوست‌ها/یادداشت‌ها = دید tabular همان جنبه برای همهٔ ردیف‌ها با ستون‌های مشترک شماره+مشتری و کلیک ردیف → مودال.
- قرارداد API: GET /api/orders?withAggregates=1 فیلدهای costsCount/costsTotal (غیرردشده)، costsApproved/costsPending، attachmentsCount را additive اضافه می‌کند — سایر مصرف‌کنندگان بدون تغییر.
- فایل‌ها: api/orders/route.ts، admin/{open-orders,open-orders-helpers,open-orders-kanban(new)}، admin/orders/{orders-page,types,use-orders-query,order-table-tabs(new),columns-by-tab(new)}. tsc صفر خطا؛ lint ۰ error (۶ warning پیش‌موجود)؛ E2E کامل سبز.

---
Task ID: 20-C
Agent: subagent-C (general-purpose)
Task: سیستم دراور و نوتیفیکیشن (فاز ۲۰) — DetailDrawer یکدست برای همهٔ نماهای جزئیات (به‌جز مودال سفارش) + نوتیف Popover→دراور + فیکس همبرگر مرده

Work Log:
- worklog (فاز ۱۷ به بعد) خوانده شد؛ مرجع بصری = دراور ۳۶۰ CRM («این شکلی خوبه ولی خوشگل‌تر و تمیزتر و مینیمالتر بشه»)؛ sheet.tsx (side left/bottom فیزیکی، دکمهٔ بستن absolute top-4 right-4) و use-mobile.ts (breakpoint 768) مطالعه شد.
- (۱) NEW src/components/ui/detail-drawer.tsx — DetailDrawer({open,onOpenChange,title,description,icon,iconClassName,widthClass,children,contentClassName}): دسکتاپ Sheet side="left" تمام‌ارتفاع با widthClass ?? "sm:max-w-xl" (لنگر فیزیکی left-0 → در RTL هم چپ می‌ماند)؛ موبایل side="bottom" با rounded-t-3xl + max-h-[92dvh] + overflow-hidden + pb-[env(safe-area-inset-bottom)] + دستگیرهٔ کشیدن وسط (w-10 h-1.5 bg-border). سربرگ داخلی: کاشی آیکون اختیاری + SheetTitle بولد + SheetDescription ریز + border-b + pr-12 برای جای X داخلی SheetContent؛ بدنه: flex-1 min-h-0 overflow-y-auto scrollbar-thin (فرزندان padding خودشان: الگوی px-5 py-4). gap-0 تا سکشن‌ها فلوش بچسبند. فقط DetailDrawer اکسپورت می‌شود.
- نکتهٔ a11y: وقتی description نباشد aria-describedby={undefined} پاس می‌شود تا هشدار Radix (Missing Description) در کنسول نیاید (کامپوننت‌های با description همچنان auto-link می‌مانند) — تست مرورگر تأیید کرد کنسول تمیز است.
- (۲) header.tsx: پاپ‌آور اعلان‌ها (Popover) → DetailDrawer (icon=bell، widthClass=sm:max-w-md). کل منطق فاز ۱۷ دست‌نخورده: کوئری ["notifications"] با polling ۱۵ث، markRead/readAll/refetch، فیلتر چیپ همه/خوانده‌نشده + ۴ نوع، آیتم‌ها (نوار رنگی + آیکون رنگی + نسبی/دقیق + کلیک→markRead+navigate+بستن)، فوتر شمارنده. ساختار جدید داخل دراور: flex h-full → نوار ابزار (بج «N خوانده‌نشده» یا «همه خوانده شده» + همه-را-خواندم + رفرش) → چیپ‌ها → لیست max-h-[65vh] → فوتر mt-auto. دکمهٔ زنگ + بج ناخوانده عین قبل (فقط onClick به‌جای PopoverTrigger).
- فیکس همبرگر مرده: toggleSidebar زامبیِ useAppStore حذف از مصرف (فیلدهای store خودشان ماندند — فایل مال من نیست) → useSidebar() از ui/sidebar + useIsMobile(): موبایل sidebar.setOpenMobile(true) (کشوی موبایل ۲۸۸px باز شد)، دسکتاپ sidebar.setOpen(!open) (gap سایدبار ۲۵۶→۴۸→۲۵۶ تست شد). آیکون/ظاهر دکمه دست نخورد.
- (۳) day-detail-modal.tsx: Dialog → DetailDrawer (نام/پروپس کامپوننت دست‌نخورده — ۴ فراخوان تقویم بدون تغییر). عنوان = روز هفته + تاریخ میلادی، توضیح = «N رویداد در این روز»، icon=calendar، widthClass=sm:max-w-2xl. هدر بزرگ مودال (عدد گرادیانتی روز) حذف و دو چیپ «فوری/یادداشت» به انتهای ردیف تب‌ها (ms-auto) منتقل شد؛ تب‌ها و ویرایشگر یادداشت سنجاق پایین (shrink-0) و بدنهٔ دوستونهٔ سایدبار آمار + محتوا در یک اسکرول واحد؛ paddings به px-5 (تب‌ها px-5 py-2، محتوا px-5 py-4، یادداشت px-5 py-3)؛ StatCards به grid-cols-2 (قبلاً lg:grid-cols-4 بر اساس viewport برای دراور خیلی تنگ بود). کل محتوا (SideStat/TimeBar/EventList/یادداشت روز) حفظ شد.
- (۴) customers-detail-dialog.tsx: Dialog/DialogContent → DetailDrawer (title «پروندهٔ مشتری»، description «سوابق سفارش‌ها، فاکتورها و پرداخت‌ها»، icon=customers، widthClass=sm:max-w-2xl). loading داخل بدنهٔ دراور ماند؛ پروفایل/۴ تایل/۳ تب/منطقهٔ خطر + AlertDialog حذف (تایپ نام) عیناً حفظ شد؛ فقط paddings به px-5 (پروفایل px-5 py-4، تایل‌ها px-5 py-4، تب‌ها px-5 pb-5، خطر px-5 py-4).
- (۵) crm-customers.tsx: Sheet/SheetHeader در CustomerDetailDrawer → DetailDrawer (title «نمای ۳۶۰ درجه مشتری»، description قبلی، icon=customers، عرض پیش‌فرض sm:max-w-xl = همان قبلی). کل سکشن‌ها بدون تغییر: پروفایل + ۳ آمار + بکس یادداشت/مانده + تب‌های سفارش‌ها (بخش‌های در جریان/پایان‌یافته + دکمهٔ فاکتور جمعی — دست‌نخورده)/معاملات/فعالیت‌ها + منطقهٔ خطر + Dialog فاکتور + AlertDialog حذف + ActivityFormDialog/DealFormDialog. موبایل خودکار بات‌شیت شد. ایمپورتهای Sheet حذف.
- راستی‌آزمایی: bunx tsc --noEmit → صفر خطا در src (فقط ۴ خطای پیش‌موجود examples/skills)؛ eslint تک‌تک ۵ فایل → ۰ error/۰ warning؛ bun run lint کل → ۰ error / ۶ warning همگی پیش‌موجود (data-table/virtualized-data-table/gantt/unused-disables — فایل‌های دیگر).
- E2E مرورگر (agent-browser سشن p20c، ادمین، 1440×900 + 390×844): نوتیف دسکتاپ → دراور چپ left=0/448px/900px با عنوان/چیپ×۷/لیست ۱۹ آیتم/فوتر؛ موبایل → بات‌شیت full-width با rounded-t-3xl + 92dvh + safe-area + دستگیرهٔ وسط (x=195)؛ همبرگر موبایل → کشوی سایدبار واقعی، دسکتاپ → جمع‌شدن (۲۵۶→۴۸px) و بازگشت؛ پروندهٔ مشتری ادمین → چپ 672px تمام‌ارتفاع + تب‌ها + منطقهٔ خطر؛ ۳۶۰ CRM → چپ 576px + پروفایل/آمار/تب‌ها/«سفارش‌های در جریان»/خطر + بات‌شیت موبایل flush-bottom؛ روز تقویم → چپ 672px عنوان «سه‌شنبه 2026/09/22» + تب‌ها + سایدبار آمار + وضعیت زمانی + ویرایشگر یادداشت. کنسول: صفر خطا/هشدار (بعد فیکس aria-describedby). اسکرین‌شات‌ها: .zscripts/p20c-{notif-desktop,notif-mobile,crm360-desktop,crm360-mobile}.png.
- نکتهٔ محیط: dev server سه بار OOM-kill شد (dmesg: next-server ~2.4GB — مشکل شناخته‌شدهٔ سندباکس با ایجنت‌های موازی؛ خودم نکشتمش) — هر بار با setsid bun run dev بک‌گراند بالا آمد؛ در پایان روشن و GET / 200 (کامپایل کامل باندل شامل هر ۵ فایل، بدون خطای compile). git دست نخورد (کامیت با ارکستراتور).

Stage Summary:
- الگوی واحد «نمای جزئیات» در کل اپ (به‌جز مودال سفارش): دسکتاپ = دراور چپِ مینیمال تمام‌ارتفاع؛ موبایل = بات‌شیت گرد با دستگیره — نوتیفیکیشن‌ها، پروندهٔ مشتری ادمین، نمای ۳۶۰ CRM و جزئیات روز تقویم همه روی همان DetailDrawer با API ثابت (title/description/icon/widthClass/contentClassName).
- نوتیف پنل همهٔ منطق فاز ۱۷ (polling/per-user read/فیلتر/رفرش/همه-را-خواندم/فوتر) را با چیدمان دراوری حفظ کرد؛ همبرگر هدر که به toggleSidebar مردهٔ Zustand وصل بود حالا سایدبار واقعی را موبایل/دسکتاپ باز-جمع می‌کند.
- فایل‌ها: NEW src/components/ui/detail-drawer.tsx؛ تغییر: layout/header.tsx، shared/day-detail-modal.tsx، admin/customers/customers-detail-dialog.tsx، crm/crm-customers.tsx. tsc صفر خطا؛ lint ۰ error (۶ warning پیش‌موجود)؛ E2E دسکتاپ+موبایل سبز؛ dev server روشن ماند.

---
Task ID: 20-E
Agent: general-purpose (mobile-responsive) + main orchestrator (fixes + QC)
Task: ریسپانسیو کامل موبایل (T1/T3 فاز ۲۰) — نمای کارتی جداول، مودال‌های تمام‌صفحه، فرم‌های داخلی، بات‌شیت‌ها

Work Log:
- data-table.tsx: prop جدید renderCard — موبایل (<768px) جدول → لیست کارتی (divide-y، کلیک = onRowClick، همان فیلتر/صفحه‌بندی)؛ منوی ستون‌ها در حالت کارتی مخفی.
- renderCard برای ۸ جدول: orders-page + open-orders (شماره/وضعیت/مشتری/آیتم‌ها/مبلغ/موعد)، crm-customers (آواتار+نام+تلفن+تعداد سفارش+چیپ مانده)، admin customers-page، finance-orders (الان کجاست/جمع/پرداخت‌شده)، print-orders (متریال/موعد چاپ)، designer-orders (موعد طراحی)، warehouse logistics-orders.
- order-detail-modal: موبایل تمام‌صفحه (w-full h-100dvh rounded-none؛ دسکتاپ عین قبل sm:max-w-5xl) + اسکرول واحد؛ فوتر safe-area (pb-[max(0.75rem,env(safe-area-inset-bottom))])؛ designer/print variants هم اصلاح.
- finance-order-modal: min-w-6xl → responsive (w-[calc(100vw-1rem)] sm:max-w-6xl)؛ جدول‌های داخلی overflow-x-auto؛ فرم‌ها grid-cols-1 sm:grid-cols-*.
- فرم‌های داخل مودال سفارش: pre-invoice-modal (PiForm ردیف‌ها grid-cols-2 sm:grid-cols-12، سقف ارتفاع)، invoice-views issue/edit (همان)، cost-entry-form responsive.
- PageHeader (shared): موبایل فشرده — توضیح hidden sm:block، آیکون size-9، mb-4.
- open-orders SummaryCards: موبایل ۴ کارت مینیمال یک‌ردیفی (grid-cols-4، بدون آیکون/hint؛ دسکتاپ عین قبل).
- header.tsx نوتیف: تراز آیکون ردیف‌ها (self-start mt-0.5).
- QC مرورگر ۳90px + VLM: داشبورد/کانبان/کارت‌ها/مودال سفارش/بات‌شیت‌ها/ویزارد/مالی — نمرات 8-9.5؛ باگ‌های عینی (هدر حجیم، safe-area، تراز) رفع؛ اسکرول‌وی افقی همه صفحات = 390 (بدون overflow).

Stage Summary:
- کل سیستم در موبایل کار می‌کند: جداول = کارت، مودال سفارش تمام‌صفحه، دراورها = بات‌شیت گرد، هدر/آمار فشرده، فرم‌ها استک می‌شوند.
- VLM نهایی: notif 9/10، kanban 8/10 (فقط اسکرول عمدی ستون‌ها + دکمه dev آرتیفکت)، cards 9/10، order modal 9/10، 360 sheets 9/10، finance 9/10، wizard 9/10.

---
Task ID: PHASE-20
Agent: main orchestrator
Task: فاز ۲۰ — هر ۱۰ خواستهٔ کارفرما: فاکتور انگلیسی جدید + نام فایل، فاکتور جمعی CRM، باگ مانده حساب CRM، حذف دور از دسترس، دراورها، کانبان، تب‌های جدول سفارشات، ریسپانسیو کامل

Work Log:
- (T9) بازنویسی کامل p24-doc.tsx — طرح انگلیسی LTR: سربرگ تیره #1C1917 + لوگو P24 طلایی + عنوان بزرگ + خط طلایی؛ BILL TO + ستون مشخصات؛ جدول با هدر تیره؛ خلاصه مالی (کاشی TOTAL تیره + باکس BALANCE DUE)؛ امضای خط‌چین؛ فوتر تیره (email/PRINTOO24.COM/phone). کامپوننت جدید P24StatementDoc برای فاکتور جمعی. تاریخ dd/MM/yyyy، IQD.
- (T10) print-doc.ts: sanitizeFileName + title = نام فایل PDF (مرورگر از title استفاده می‌کند) → «Invoice No_ N - Customer» / «Quotation No_ N - Customer» / «Invoice - Customer - Active Orders»؛ پنجره چاپ lang=en dir=ltr.
- (T6) crm-customers.tsx تب سفارش‌ها: بخش «سفارش‌های در جریان» + دکمه «چاپ فاکتور سفارشات جاری» + ۳ کاشی جمع/پرداخت/مانده + خط جداکننده + «پایان‌یافته/آرشیو» زیر خط؛ مودال استیتمنت با P24StatementDoc (فقط سفارش‌های active — نه تکمیل/آرشیو/لغو)؛ OrderRow با StatusBadge + پرداخت/مانده.
- (T7) باگ مانده حساب CRM: balanceDue مرده → unsettled زنده (ستون جدول + باکس بدهی دراور) — همان فیلد additive ادمین.
- (T8) حذف مشتری: دکمه از ردیف‌های CRM (confirm مرده) و ادمین (AlertDialog) حذف شد → «منطقهٔ خطر» انتهای پرونده/۳۶۰ با AlertDialog تایپ-نام-مشتری (disabled تا تطابق دقیق).
- (T2/T3) ایجنت 20-C: ui/detail-drawer.tsx (دسکتاپ چپ / موبایل بات‌شیت rounded-t-3xl + هندل + safe-area)؛ نوتیف Popover → DetailDrawer؛ همبرگر مرده → useSidebar واقعی؛ day-detail-modal و پروندهٔ مشتری ادمین → دراور؛ ۳۶۰ CRM → DetailDrawer.
- (T4/T5) ایجنت 20-D: api/orders?withAggregates=1 (costsCount/Total/Approved/Pending + attachmentsCount)؛ سفارشات باز: سوییچ «نمای جدول|نمای کانبان» (dnd-kit، ۳ ستون وضعیت، درگ→PUT status با rollback)؛ هر دو صفحهٔ سفارشات: تب‌های «همه|آیتم‌ها|هزینه‌ها|پیوست‌ها|یادداشت‌ها» با ستون‌های اختصاصی.
- (T1/T3) ایجنت 20-E + main: renderCard برای ۸ جدول، مودال سفارش تمام‌صفحهٔ موبایل + safe-area، مودال مالی responsive، فرم‌های ویزاردی استک، PageHeader/SummaryCards فشرده.
- QC مرورگر (agent-browser 390px + 1440px) + VLM روی ۱۵ اسکرین‌شات: پنجره چاپ فاکتور 9.5/10 (فقط خود سند، ساختار کامل)؛ Quotation 8/10 (اسکژول شرطی به داده)؛ استیتمنت 9/10؛ کانبان/کارت‌ها/مودال‌ها/بات‌شیت‌ها 8-9/10؛ عنوان پنجره چاپ تأیید: «Quotation No_ 10 - داروخانه سبز» و «Invoice - ساختمانی آرمان - Active Orders»؛ تب‌های جزئیات (هزینه: تعداد/جمع/تایید/در انتظار/سود برآوردی؛ آیتم؛ پیوست؛ یادداشت) همه دیتای واقعی.
- تیپ دیالوگ استیتمنت (sr-only DialogTitle) بعد از هشدار Radix اضافه شد؛ tsc/lint تمیز (۰ خطای src؛ ۶ وارنینگ پیش‌موجود).
...
---
Task ID: PHASE-22.5
Agent: main orchestrator (new hire — session 2)
Task: رفع دو باگ کشف‌شده در تست مجدد فاز ۲۲ (هدیه × فاکتور) + راه‌اندازی مجدد محیط بعد از ریست سندباکس

Work Log:
- محیط بعد از ریست سندباکس بازسازی شد: clone از GitHub، bun install، Prisma generate؛ دیتابیس زنده به مسیر پیش‌فرض سندباکس (my-project/db/custom.db) منتقل شد تا DATABASE_URL تزریقیِ پلتفرم با CLI و runtime هم‌راستا باشد (رفع دائمی تلهٔ «دیتابیس شبح»).
- اجرای test-phase22: یک assertion شکست خورد → ریشه‌یابی دو باگ واقعی در تعامل هدیه×فاکتور:
  - باگ ۱ (حیاتی — نقض خواستهٔ ۶): صدور/ویرایش/تبدیل فاکتور روی سفارشِ هدیه‌شده، syncOrderTotalFromInvoice هدیه را از total پاک می‌کرد → مشتری دوباره بدهکار می‌شد. رفع: هدیهٔ فعال موقع POST /api/invoices و convert داخل discount خود سند «تاخته» می‌شود (همان قرارداد endpoint هدیه بعد از صدور)؛ سند چاپی هم بدهی واقعی را نشان می‌دهد؛ sync بدون تغییر.
  - باگ ۲: مبنای درصد هدیهٔ مجدد «itemsSum + هدیهٔ قبلی» بود → re-gift درصد/مبلغ را متورم می‌کرد (100% روی 960k بعد از re-gift می‌شد 1,920k). رفع: مبنای خام = itemsSum (بدون فاکتور) یا invoice.subtotal (با فاکتور).
- test-phase22.mjs بازنویسی: ۶-الف فاکتور روی سفارش هدیه‌شده (doc total=0 + مشتری بدون بدهی + هدیهٔ پابرجا بعد از ابطال) + ۶-ب سینک خالص تخفیف روی سفارش تمیز؛ خودترمیمی فاکتورهای مانده از اجرای قبلی.
- تست کامل: همهٔ ۲۲ assertion سبز (هدیه 960,000 درست، doc total=0، سینک 60,000=75,000−15,000، برگشت اقلام بعد از حذف، radar.lossOrders).
- QC مرورگر: لاگین بدون یوزرنیم/پسورد پیش‌پرشده (خواستهٔ ۱۱ تأیید)، رادار رئیس با کارت «سفارش‌های زیان‌ده»، مودال سفارش #7: ردیف هزینه/قیمت/سود/موعد + کاشی پرداختی/باقیمانده + نشان «هدیه ثبت شده» (باقیمانده 0)؛ کنسول بدون خطا؛ اسکرین‌شات‌ها در .zscripts/p22qc-*.png.

Stage Summary:
- دو باگ مالی هدیه×فاکتور رفع شد؛ رفتار صحیح: مشتری هدیه‌شده هرگز بدهکار نمی‌شود (حتی بعد از صدور فاکتور) و سند فاکتور هم بدهی واقعی (با تخفیف هدیه) را نشان می‌دهد. tsc تمیز؛ تست سبز؛ آمادهٔ دیپلوی فاز ۲۲+۲۲.۵ روی production.

---
Task ID: PHASE-22-DEPLOY
Agent: main orchestrator (new hire — session 2)
Task: استقرار فاز ۲۲+۲۲.۵ روی سرور production (پروژهٔ خودمان — بدون دست زدن به Docker)

Work Log:
- بکاپ کامل دیتابیس واقعی قبل از هر تغییری: db/custom.db.bak-phase22 (md5 یکسان، integrity_check: ok — ۱۱ سفارش/۸ کاربر/۸ مشتری/۵ فاکتور/۱۱ پیش‌فاکتور/۲۱ هزینه).
- تاربال ۳۱ فایل (بدون دیتابیس!) — فقط سورس P22+P22.5 (سرور روی P21 بود) → آپلود → استخراج در /opt/printoo24-admin. کانتینرهای Docker پروژهٔ دیگر دست‌نخورده.
- npx prisma generate → توقف سرویس → db push (۵ ستون gift روی Order — افزایشی، دیتا دست‌نخورده) → backfill-phase22: ۳ سفارش سینک با فاکتور (#1: −15,000 / #2: −30,000 / #3: −57,200 — تخفیف‌های فاکتور بالاخره روی بدهی مشتری نشست).
- NODE_ENV=production npm run build → BUILD_EXIT=0 → systemctl start → active.
- تأیید production: GET / 200 و HTML بدون admin@printoo24/admin123 (رفع نشت رمز — خواستهٔ ۱۱)؛ مرورگر: اینپوت‌های لاگین خالی، باکس «دسترسی دمو» حذف شده؛ POST بدون کوکی → 401 روی همهٔ مسیرها (از جمله gift)؛ لاگین admin 200؛ /api/dashboard → radar با ۶ کارت از جمله lossOrders: {"count":1,"sum":62060,"top":[{"name":"#8 — Evan","due":62060}]} — زیان واقعی جلوی چشم رئیس.
- پاک‌سازی فایل‌های موقت سرور؛ journalctl بدون خطا.
...
Task: ۷ خواستهٔ جدید کارفرما روی پوش «تغییرات ظاهری» — (۱) هزینهٔ خودِ مالی بدون صف تأیید، (۲) سفارش تمام‌شده بدون زمان/موعد، (۳) حذف کامل اعداد فارسی و تاریخ شمسی، (۴) انتخاب خودکار محصول جدید در ویزارد، (۵) نمای ۳۶۰ + فاکتور جمعی در مدیریت مشتریان ادمین، (۶) دسترسی ۳لایه مشاهده/ادیت/حذف برای هر ماژول، (۷) بازنویسی کامل تولتیپ‌های آموزشی.

Work Log:
- محیط: پوش کاربر (22a691c «تغییرات ظاهری») کلون شد — تغییرات: عرض مودال‌ها + ترتیب تب‌های چاپ + دیتابیس تازه (prisma/db/custom.db). دیتای تازه به مسیر تزریقی پلتفرم (/home/z/my-project/db/custom.db) منتقل شد.
- ۱) POST /api/material-costs: گیت status ساده شد — finance ? approved : pending. هزینهٔ ثبت‌شدهٔ خودِ مالی (هر ماژولی: آزاد/روی سفارش/فاکتوری) مستقیم approved؛ ماژول‌های دیگر مثل قبل pending تا تأیید مالی.
- ۲) isOrderClosed() در lib/format (completed|archived|cancelled) — orders-columns فقط تاریخ، مودال جزئیات سفارع بدون hint/tone زمان، مودال چاپ کاشی «باقی‌مانده»→«تکمیل شده/لغو شده/آرشیو»، کارت موبایل بدون موعد. رادار/برد فقط آیتم‌های فعال — بدون تغییر.
- ۳) اسکریپت p24-remove-persian-digits.py روی ۱۳۰ فایل: "fa-IR"→"en-US" (تقویم شمسی هدر + اعداد)، ارقام فارسی ۰-۹→ASCII در همهٔ رشته‌ها، مبدل faDigits (۳ فایل payroll) حذف، toFa→fmtNum. localeCompare("fa") برای مرتب‌سازی نام فارسی دست‌نخورده. backfill-phase24.mjs: ۱۲ اعلان + ۲ رویداد سفارش با ارقام لاتین مهاجرت شد.
- ۴) ویزارد: newItem قیمت می‌گیرد، addItem(cid, prefill) — محصول تازه‌ساخت خودکار در سطر جدید انتخاب + قیمت پایه پیش‌پر (مثل مشتری). API products از قبل full object برمی‌گرداند.
- ۵) components/shared/customer-360-drawer.tsx (دراور واحد): پروفایل+آمار، ۵ تب (سفارش‌ها با بدهکار/تسویه + فاکتور جمعی چاپی، فاکتورها، پرداخت‌ها، معاملات، فعالیت‌ها)، منطقهٔ خطر. CRM (−۵۵۲ خط) و مدیریت مشتریان ادمین هر دو از آن استفاده می‌کنند؛ customers-detail-dialog حذف شد؛ /api/customers/[id] orders با items[].product.name غنی شد.
- ۶) دسترسی ۳لایه: UserModule.level (view|edit|delete — دیفالت delete = سازگار)، proxy.ts گیت مرکزی با ROUTE_MODULES (نگاشت سگمنتی route→ماژول‌های مالک + max-rank) — view همهٔ نوشتاری‌ها ۴۰۳، edit فقط DELETE ۴۰۳؛ کوکی در login/heartbeat/me با moduleLevels تازه prime می‌شود (حداکثر ۴۵ث تأخیر). users API (POST/PUT) validateModuleLevels + serializeLevel. UI: ModuleLevelPanel (۳ گزینهٔ رنگی + hint) در هر دو فرم کاربر (sysadmin monitoring + users-page legacy) + بج سطح در فهرست.
- ۷) guide-content.ts بازنویسی کامل: ماژول‌ها/صفحات با نقش کامل هر بخش، BUTTON_CTX بافت‌آگاه («module:page|متن» — دکمه‌های هم‌نام در صفحات مختلف توضیح مخصوص)، سرستون‌ها/فیلدهای غنی‌شده، رادار رئیس هر ۶ کارت با کلید خودش، c360 برای دراور. resolver: بافت از app-store → BUTTON_CTX → دیکشنری → smartButtonFallback (تحلیل فعل، جملهٔ کامل — نه «دکمه است کلیک کن»). data-guide به کارت‌های رادار و دراور ۳۶۰ وصل شد.
- تست: scripts/test-phase24.mjs — ۳۳/۳۳ سبز (هزینه مالی approved، بدون ارقام فارسی، prefill محصول، items در ۳۶۰، سناریوی کامل view→edit→delete با heartbeat re-prime و PUT مستقل). regression: test-phase23-demo ۱۹/۱۹، test-phase22 همه ✓. tsc/eslint/build پروداکشن پاک. QC مرورگر: لاگین، داشبورد بدون ارقام فارسی، تولتیپ ماژول مالی (متن جدید)، صفحه مشتریان ادمین + دراور ۳۶۰ با ۵ تب + فاکتور جمعی برای مشتری بدهکار (بیمارستان مهر)، پنل سطح در فرم ویرایش کاربر (تعاملی)، آرشیو/تکمیل‌شده‌ها بدون شمارش معکوس، ویزارد: محصول «P24 AutoSelect Test» بعد از ساخت خودکار انتخاب شد (سپس پاک‌سازی).
...
---
Task ID: PHASE-25.1-FX-MARKET
Agent: main orchestrator (session 8 — ادامه)
Task: فاز ۲۵.۱ — نرخ‌های ارز کاملاً غلط بودند (open.er-api رسمی ۱۳۰۸ IQD/۱۴۴٬۳۲۴ تومان در برابر بازار واقعی ۱۵۶۱/۲۳۴٬۶۱۵) → API حرفه‌ای بازار TGJU + مرور کامل منطق محاسباتی + QC کامل + راستی‌آزمایی با سرچ

Work Log:
- تشخیص با سرچ وب: بازار تهران (TGJU ۲٬۳۴۶٬۱۵۰ ریال، alanchand ۲۳۵٬۴۰۰ تومان) و بازار دینار (~۱,۵۶۵ سلیمانیه/بغداد) — نرخ API قبلی ۱۶٪ (دینار) و ۳۸٪ (تومان) خطا داشت؛ نرخ رسمی بانک مرکزی مبنای کارگاه نیست.
- منبع جدید: call1.tgju.org/ajax.json (بازار آزاد، بدون کلید، دقیقه‌ای) — USD_IRT = price_dollar_rl÷۱۰ و USD_IQD = price_dollar_rl÷price_iqd (نرخ متقاطع بازار — منطبق بر بورصات سلیمانیه/بغداد در ۰.۳٪) + فالبک رسمی open.er-api فقط وقتی بازار ۷۲س در دسترس نیست + سیید اضطراری به‌روزشده به نرخ بازار.
- باگ‌های منطق محاسباتی که پیدا و رفع شد: (۱) payroll-pay.ts اسنپ‌شات دوره با هاردکد ۱۳۱۰/۱۵۰۰۰۰ بود → نرخ زنده؛ (۲) فالبک‌های هاردکد summary/payroll routes → FX_SEED؛ (۳) iqdPerIrt (۱۰۰۰ تومان=X دینار، رند صحیح روی عدد کوچک) → irtPerIqd «۱ دینار=X.X تومان»؛ (۴) FX_SEED کهنه ۱۳۱۰/۱۵۰۰۰۰ → ۱۵۶۰/۲۳۴٬۶۰۰؛ (۵) نرخ دستی بعد از ۱ ساعت سایه می‌شد → «چسبنده» + دکمه «بازگشت به خودکار» (POST {clear})؛ (۶) جمع‌های منفی در formatSumPerCurrency مخفی می‌شدند؛ (۷) باگ مبنای زمان: fetchedAt ردیف بازار با زمان تهران quote می‌شد که از ردیف‌های now قدیمی‌تر sort می‌شد (نرخ نامرئی) → مبنای واحد زمان فچ سرور + dedupe ضد-spam (±۰.۵٪ همان منبع = بدون ردیف) + رفرش ۱۵دقیقه‌ای + باندهای سلامت (IQD ۹۰۰-۳۰۰۰ / IRT ۶۰k-۶۰۰k).
- UI: پنل/FxBar/گیت چاپ/نوار اسناد همه منبع را برچسب می‌زنند («بازار (TGJU)»/«رسمی»/«دستی»)؛ اسناد چاپی: «MARKET EXCHANGE RATE — TGJU»؛ پنل نرخ توضیح منبع + هشدار ۱۵٪ اختلاف رسمی/بازار.
- تست: test-phase25.mjs بازنویسی بخش FX — ۵۳/۵۳ سبز شامل راستی‌آزمایی مستقیم با فچ TGJU (تطابق ±۲٪)، دستی چسبنده، clear، بازگشت بازار. رگرسیون: phase24 ۳۳/۳۳ (رقم فارسی بقایای تست فاز۲۲ پاک شد)، phase23 ۱۹/۱۹، phase22 ✓. tsc/eslint پاک.
- دیپلوی ایمن روی production: بکاپ custom.db.bak-phase251-20260924-1655 → snapshot ردیف‌ها → تاربال ۱۰ فایل سورس (بدون db/env) → دو راند build/restart (رفع باگ مبنای زمان در راند دوم) → راستی‌آزمایی: PROD RATES دقیقاً بازار (۱٬۵۶۱ IQD / ۲۳۴٬۶۱۵ IRT / منبع market / age ۰) — تطابق کامل با TGJU زنده.
- QC مرورگر زنده prod (فقط-خواندنی): پنل «نرخ لحظه‌ای ارز — بازار (TGJU) · تازه» با هر سه نرخ؛ KPIهای واقعی دست‌نخورده؛ پیش‌فاکتور #۲۴: نوار MARKET EXCHANGE RATE — TGJU روی سند؛ گیت چاپ: ۳۹۰٬۰۰۰ IQD → ۲۴۹.۸۴ USD / ۵۸٬۶۱۶٬۱۷۶ تومان (ریاضی دقیق با نرخ غیررند)؛ سند USD کامل با یادداشت Converted...Original total. صفر خطای کنسول.
- راستی‌آزمایی نهایی با سرچ تازه: کانال بورصات عراق ۱٬۵۶۵-۱٬۵۶۶ / TGJU ۱٬۵۶۱ / سیستم ۱٬۵۶۱ (تطابق)؛ تومان: TGJU ۲۳۴٬۶۱۵ = سیستم؛ NRT/کتایب قدیمی‌تر ~۱٬۵۱۴ (روزِ متفاوت) — سیستم روی خوشهٔ زنده است.
- سلامت دیتا: diff شمار ردیف‌ها قبل/بعد IDENTICAL (فقط fxRate کش +۴). Docker دست‌نخورده.

Stage Summary:
- نرخ‌های سیستم حالا نرخ واقعی بازار است: ۱ دلار = ۱٬۵۶۱ دینار / ۲۳۴٬۶۱۵ تومان / ۱ دینار = ۱۵۰.۳ تومان — از TGJU (بازار آزاد، رفرش ۱۵دقیقه) با فالبک رسمی و سیید؛ دستی چسبنده با دکمه بازگشت؛ همهٔ تبدیل‌ها/اسناد/جمع‌ها/اسنپ‌شات‌ها با نرخ زنده بازار. باگ‌های هاردکد و مبنای زمان رفع شد. دیتای واقعی ۱۰۰٪ سالم.

---
Task ID: PHASE-26-BULK-SETTLE
Agent: main orchestrator (session 9)
Task: خواستهٔ کاربر: «مشتری ۵ سفارش تسویه‌نشده/نیمه‌تسویه دارد، یک‌جا پول می‌دهد، باید تک‌تک سفارش‌ها را باز کنم و پر کنم — اذیت‌کننده است. می‌خواهم مبلغ را وارد کنم تا بدانم چقدر داده و چقدر مانده؛ ثبت راحت و بدون مشکل در سیستم» → فاز ۲۶: تسویه گروهی بدهی

Work Log:
- طراحی: دیالوگ «تسویه گروهی» — انتخاب مشتری بدهکار (مرتب بر اساس بدهی، با عدد بدهی در زیرنویس) → چیپ ارز (فقط ارزهای بدهکار؛ دیفالت بیشترین بدهی) → مبلغ دریافتی + روش (نقدی/کارت/چک) + یادداشت → جدول تخصیص FIFO قدیمی‌ترین‌اول با ویرایش دستی هر ردیف (صفر = رد نشود) + دکمه «بازچین خودکار» → ثبت یک‌جا → نمای نتیجه (تخصیص هر سفارش، تسویه کامل ✓/مانده، بدهی باقی‌ماندهٔ همهٔ ارزها).
- API جدید POST /api/finance/bulk-settle: قواعد سروری — ماندهٔ تازه از DB داخل تراکنش (کلمپ ضد ثبت هم‌زمان)؛ بدون allocations → FIFO خودکار؛ با allocations → کلمپ به مانده + ترم Σ>amount از انتهای لیست؛ مازاد (excess) گزارش می‌شود و ثبت نمی‌شود (بدهی هرگز منفی نمی‌شود)؛ فقط سفارش‌های همان ارز؛ همهٔ ردیف‌ها در یک db.$transaction اتمیک؛ هر ردیف از همان applyPaidAmountChange موجود رد می‌شود (RevenueLog + توزیع پیش‌فاکتور + آینهٔ فاکتور + رویداد حساس payment_recorded با عنوان «تسویه گروهی») — صفر مسیر حسابداری جدید. دسترسی: مستر/مالی/مدیر/انبار (همان قاعدهٔ ثبت دریافتی).
- UI: src/components/shared/bulk-settle-dialog.tsx (مرحله‌ای + جمع زندهٔ «مبلغ/ثبت می‌شود/بدهی بعد از تسویه/ردیف‌ها» + هشدار amber مازاد + یادآوری ارزهای دیگر + Invalidate خودکار سفارش‌ها/درآمد/مالی/داشبورد/مشتری‌ها). نقاط ورود: دکمهٔ «تسویه گروهی» در سربرگ صفحهٔ تسویه‌نشدهٔ مالی + دکمهٔ «تسویه گروهی بدهی» کنار «چاپ فاکتور سفارشات پرداخت‌نشده» در دراور ۳۶۰ مشتری (با preset مشتری).
- تست: scripts/test-phase26.mjs — ۴۹/۴۹ سبز: FIFO دقیق (100k/150k روی ۲ سفارش)، سینک پیش‌فاکتور/فاکتور، RevenueLog ارزی + یادداشت، رویدادهای حساس، کلمپ سروری (200k→80k)، ترم Σ>amount، excess (285k ثبت/715k مازاد)، تفکیک ارزی USD، اعتبارسنجی 400/404/409/401، یکپارچگی نهایی + پاک‌سازی کامل. رگرسیون: phase24 ۳۳/۳۳، phase25 ۵۳/۵۳. tsc/eslint پاک.
- QC مرورگر سندباکس (دیتای تستی موقت): سناریوی دقیق کاربر (۵ سفارش: ۴ بدهکار + ۱ تسویه‌شده + ۱ USD) — پیکر بدهکارها مرتب، چیپ ارز، FIFO خودکار 450k+200k+150k، ویرایش دستی (300k بدون بازچین)، «بازچین خودکار» بازگشت، ثبت 800k → نتیجه با «تسویه کامل ✓»×2 + مانده، جدول اصلی خودکار رفرش (ردیف‌های تسویه‌شده حذف شدند)، ورودی ۳۶۰ با بدهی به‌روزشده، تسویه نهایی 450k → بدهی IQD صفر/«200 USD» باقی. صفر خطای کنسول. ۵ اسکرین‌شات research/p26qc-*.png.
- 🔴 INCIDENT + بازیابی کامل: اسکریپت قدیمی rowcount.mjs روی سرور آرگومان را «مسیر فایل خروجیِ snapshot» می‌دانست نه ورودی DB — اجرای BEFORE من با مسیر DB، خروجی متن شمارش را داخل custom.db زنده نوشت (۲۶۴ بایت ASCII). بکاپ ۲۰:۱۹ (custom.db.bak-phase26-20260925-1015، ۶۴۳KB — دقیقاً چند ثانیه قبل از آسیب) با cp بازگردانی شد + integrity_check=ok + شمارش‌ها مو‌-به‌مو با snapshotِ داخل فایل آسیب‌دیده یکسان (user=12, customer=14, order=22, orderItem=38, invoice=11, preInvoice=22, materialCost=25, revenueLog=9, notification=49, orderEvent=102, userModule=21, payment=0, activity=0, deal=0, task=0, supplier=2, material=0, package=0, payrollEntry=8, expenseType=6) → صفر دادهٔ واقعی از دست رفت (پنجرهٔ آسیب <۶۰ ثانیه و همهٔ نوشتن‌ها بعد از آن fail می‌شدند). درس: از این پس فقط scripts/server/rowcount-py.py (sqlite3 با mode=ro) برای شمارش روی prod؛ هیچ اسکریپت سروری با آرگومان-path بدون خواندن سورسش اجرا نمی‌شود.
- دیپلوی ایمن: بکاپ → (بازیابی incident) → تاربال ۷ فایل سورس (بدون db/env) → استخراج → build پروداکشن npm (exit 0، route.js فاز ۲۶ در standalone تأیید) → restart → راستی‌آزمایی فقط‌خواندنی ۱۳/۱۳ (prod-verify-phase26.mjs: لاگین/401/400×۳/404/409 «این مشتری بدهی باز به دینار عراقی ندارد»/fx/summary/orders/heartbeat/نرخ بازار ۱۵۶۱) → QC مرورگر prod بدون mutation (پیکر مشتری‌های واقعی با بدهی، دیالوگ Rawa reklam با ۸ سفارش واقعی، FIFO 500k → 150k+90k+189k+65k+6k دقیق، بسته بدون ثبت) → diff شمار ردیف‌ها قبل/بعد IDENTICAL → پاک‌سازی فایل‌های موقت سرور.

Stage Summary:
- فاز ۲۶ فعال روی production (http://187.124.27.96:3000): تسویه گروهی از صفحهٔ تسویه‌نشدهٔ مالی و دراور ۳۶۰ مشتری — یک موج پرداخت، تخصیص FIFO خودکار/دستی اتمیک، مازاد شفاف، نتیجهٔ «چقدر داده/چقدر مانده» per سفارش و per ارز. دیتای واقعی ۱۰۰٪ سالم (incident دیتابیس کامل بازیابی شد و شمارش‌ها قبل/بعد یکسان). Docker سایت مشتریان دست‌نخورده.
